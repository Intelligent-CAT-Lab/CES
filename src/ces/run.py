...
from prompts import chatgpt_generator, huggingface_generator, huggingface_generator_chat, generator_gemini, generator_lllm
...
def read_loop_data(dataset):
    ...
    iterator_path_orig = os.path.join(root, f"{dataset}_loop.json")
...
def read_condition_data(dataset):
    ...
    condition_path_org = os.path.join(root, f"{dataset}_condition.json")
    condition_data_org = json.load(open(condition_path_org, 'r'))
    
    condition_data.update(condition_data_org)
...
def read_branch_data(dataset):
    ...
    branch_path_org = os.path.join(root, f"{dataset}_branch.json")
    branch_data_org = json.load(open(branch_path_org, 'r'))
    branch_data.update(branch_data_org)
...
def main(dataset, pl, model_id, cache_dir, write_dir, direct, no_nl):
    ...
    write_root = os.path.join(write_dir, 'adaptive_loop_condition_branch', model_id.split("/")[-1], dataset)
    if no_nl:
        write_root = os.path.join(write_dir, 'adaptive_loop_condition_branch','no_nl', model_id.split("/")[-1], dataset)
    if direct:
        write_root = os.path.join(write_dir, 'adaptive_loop_condition_branch', 'direct', model_id.split("/")[-1], dataset)
