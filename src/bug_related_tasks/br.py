...
from create_prompt_br import create_prompt
from prompts import chatgpt_generator, generator_lllm, generator_vllm
import json
...
def main(model_id, dataset, cache_dir, write_dir, with_tests):
    ...
    if with_tests:
        write_root = os.path.join(write_dir, 'BR', model_id.split("/")[-1], dataset)
    else:
        write_root = os.path.join(write_dir, 'BR', model_id.split("/")[-1], dataset)
